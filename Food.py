...
def render_nutrient_control(label, prefix, col_name, emoji):
    ...
    with st.container(border=True):
        ...
        if is_active:
            ...
            if prefix == 'pro' and val >= 12.0:
                st.success("💪 **High Protein**")
            elif prefix == 'fat' and val <= 3.0:
                st.success("🟢 **Low Fat**")
            elif prefix == 'sugar' and val <= 5.0:
                st.success("🟢 **Low Sugar**")
            elif prefix == 'sod' and val >= 600:
    ...
    if not active_features:
        ...
        with st.spinner('Calculating...'):
            ...
            for i in range(k_val):
                ...
                with st.container():
                    st.markdown(f"<h3 style='color: #1E3A8A;'>Rank {i + 1}: {row['food_name']}</h3>",
                                unsafe_allow_html=True)
                    cols = st.columns(3)
